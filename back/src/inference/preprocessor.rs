...
pub fn preprocess_geometry(
    ...
    for block in geometry {
        let gx = (block.x as usize).min(nx - 1);
        let gy = (block.y as usize).min(ny - 1);
        let gz = (block.z as usize).min(nz - 1);
...
fn surface_properties(surface_type: &str) -> (f32, f32, f32, f32, f32) {
    match surface_type {
        "concrete" => (0.3, 0.90, 1.4, 0.02, 0.0),
        "asphalt" => (0.1, 0.95, 0.75, 0.01, 0.0),
        "glass" => (0.4, 0.84, 1.0, 0.001, 0.0),
        "vegetation" | "grass" => (0.25, 0.95, 0.3, 0.1, 1.0),
        "water" => (0.06, 0.96, 0.6, 0.0001, 0.0),
